...
impl Asfaltinator {
    ...
    pub fn asfalting(
        ...
        for direction in sequence.iter_mut() {
            ...
            let wheeeere = map[((robot.get_coordinate().get_col() as i32) + shift.0) as usize]
            ...
            let where_is_going = map[((robot.get_coordinate().get_col() as i32) + shift.0) as usize]
                [((robot.get_coordinate().get_row() as i32) + shift.1) as usize]
                .clone();
            let res = match where_is_going {
                ...
                | Some(ref x) => {
                    let mut put_result = match x.tile_type {
                        ...
                        | TileType::Street => Ok(1),
                        | TileType::Mountain => {
                            // destroy(robot, world, direction.clone())?; ???
                            put(robot, world, Content::None, 1, direction.clone())
                        }
                        | TileType::Teleport(_) | TileType::Wall => {
                            return Err(UnFinishedProject::new(
                                project,
                                StopReason::PonteSulloStretto,
                            ...
                            match destroy(robot, world, direction.clone()) {
                                | Ok(_) | Err(LibError::NotEnoughSpace(_)) => {
                                    ...
                                    put_result = match wheeeere.tile_type {
                                        ...
                                        | TileType::Sand | TileType::Grass | TileType::Hill | TileType::Snow => {
                                            put(robot, world, Content::Rock(0), 1, direction.clone())
                                        }
                                        | TileType::Mountain => put(robot, world, Content::None, 1, direction.clone()),
                                        | x => {
                                            print!(
                                                "it is missing this unexpected case
                                        scenario on this tiletype, after it destroyed its content {:?}",
                                                x
                                            );
                                            //Err fittizio   |
                                            //                  |
                                ...
                                | Err(x) => {
                                    print!("it is missing this unexpected case scenario {:?}", x)
                                }
                            }
                        }
                        | _ => {}
            ...
            if go_result == Err(CannotWalk) || go_result == Err(OutOfBounds) {
                return Err(UnFinishedProject::new(
                    project,
                    StopReason::PonteSulloStretto,
                    (robot.get_coordinate().get_row(), robot.get_coordinate().get_col()),
                ));
            } //ive just built a road so i must be able to walk on it
            if go_result == Err(NotEnoughEnergy) {
                return Err(UnFinishedProject::new(
                    project,
                    StopReason::LowEnergy,
                    (robot.get_coordinate().get_row(), robot.get_coordinate().get_col()),
